...
| `src/styles/`       | 按级联顺序拆分的基础、工作台和教程样式       | 只使用共享设计 token               |
...
之后加载。样式结构门禁位于 `tests/config/style-manifest.test.ts`。

教程 UI 使用单向依赖：课程控制器消费独立 contracts 与源码锚点模块；数据流控制器再
分别消费 presentation 和 geometry，后两者不得反向导入控制器。原门面文件继续
re-export 公共类型，避免拆分迫使调用方迁移或形成循环依赖。

## 注释约定

- 注释解释设计原因、所有权、失败边界或平台限制，不逐行复述实现。
- 公共接口只记录调用方必须知道的契约；能由类型和命名直接表达的内容不再重复。
- `fail-closed`、源码权威、证据可信度和异步销毁检查属于安全边界，修改代码时必须同步更新说明。
- CSS 分区注释用于标明级联职责；不得把历史版本号或临时修补过程当作长期文档。
- 过时注释应与代码一同删除；禁止用注释保留已经失效的实现。
